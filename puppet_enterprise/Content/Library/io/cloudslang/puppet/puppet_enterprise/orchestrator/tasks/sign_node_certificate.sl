########################################################################################################################
#!!
#! @description: Sign the certificate for a new node in Puppet Enterprise
#!
...
#! @input node: Certificate name of hostname of the new node to sign the certificate. Example: pupnode3.example.com
#!
#! @output job_id: List of Puppet Enterprise groups
#!!#
########################################################################################################################
namespace: io.cloudslang.puppet.puppet_enterprise.orchestrator.tasks
flow:
  name: sign_node_certificate
  inputs:
    - PuppetEnterpriseURL
    - PuppetUsername
    - PuppetPassword:
