########################################################################################################################
#!!
#! @input start_date: Format is YYYY-MM-dd HH:mm:ss
    ...
    - sql_command:
        do:
          io.cloudslang.base.database.sql_command:
            ...
            - db_port: '5432'
            - database_name: power
            - command: "${'''\nselect timestamp BETWEEN \\''''+start_date+'''' AND \\''''+stop_date+'''', round(sum(cast(\"cost\" as float8))::numeric,2) \"usage_cost per period\"\nFROM \"table\" t\ngroup by 1\nORDER BY 1\n;\n'''}"
    ...
    - get_values_v2:
        do:
          io.cloudslang.base.maps.get_values_v2:
            ...
            - d: "${start_date+' and '+stop_date}"
        publish:
          - energy_cost: "${'The costs of enery between '+d+' was €'+return_result}"
        navigate:
          - SUCCESS: SUCCESS
          - FAILURE: on_failure
  outputs:
